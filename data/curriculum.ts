  ...
  },
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        source: secRisk,
        chartContext: "Use a band or zone around repeated reactions rather than a single exact price. Note how many prior observations are relevant and whether the current timeframe matches the learning question.",
        practicePrompt: "Draw one upper and one lower zone on a fictional range. Write what would count as acceptance, rejection, and an inconclusive result.",
        limitation: "Prior reaction areas can fail, be crossed without follow-through, or become less relevant when liquidity, information, or the timeframe changes.",
      },
      {
        ...
        source: secRisk,
        chartContext: "Record the indicator name, input series, lookback length, smoothing convention, and timeframe. Those settings are part of the observation rather than background details.",
        practicePrompt: "Compare one price-structure observation with one indicator reading in a fictional chart. List one reason the two may not align.",
        limitation: "An indicator transforms past inputs. It cannot remove event risk, execution friction, or uncertainty about the next price change.",
      },
      {
      ...
      },
      {
        ...
        title: "Ranges, breakouts, and failed moves",
        duration: "8 min",
        objective: "Describe a range transition and its uncertainty without assuming every break becomes a sustained move.",
        body: "A range is a period in which price repeatedly trades between an upper and lower area. A breakout is a move through a watched area, but it is not complete evidence by itself. A useful learning plan defines the range, the observation that would count as follow-through, the observation that would count as a return inside the range, and the risk boundary for either interpretation.",
        takeaways: ["Ranges describe repeated containment, not permanent balance.", "A breakout can fail or lack follow-through.", "Confirmation criteria should be stated before interpreting a break."],
        quiz: { question: "What is a failed breakout in chart vocabulary?", options: ["A move outside a range that returns inside it", "A guaranteed reversal", "A stock split", "A lower commission"], answerIndex: 0, explanation: "A failed breakout describes a move beyond a watched area that later returns within it; it does not guarantee what follows." },
        source: cmeTechnical,
        chartContext: "Draw the upper and lower range zones, then document whether the close, range expansion, time outside the zone, or volume comparison is part of the study rule.",
        practicePrompt: "Define a fictional range and write separate observations for acceptance above it, rejection back inside it, and no clear evidence.",
        limitation: "No universal rule determines whether a range break is valid. The evidence and timeframe must be specified in advance.",
      },
      {
