...
const cftcGlossary: ReferenceSource = { label: "CFTC — Futures Glossary", url: "https://www.cftc.gov/LearnAndProtect/AdvisoriesAndArticles/CFTCGlossary/index.htm" };
const cmeTechnical: ReferenceSource = { label: "CME Group — Technical Analysis Course", url: "https://www.cmegroup.com/education/courses/technical-analysis.html" };
...
export const referenceDomains: ReferenceDomain[] = [
  ...
  {
    id: "technical-analysis", title: "Technical analysis", description: "A source-linked, education-only study lane for reading chart structure, price, volume, patterns, and indicator limitations without treating them as predictions.", accent: "#4666B0", topics: [
      topic("chart-construction", "Chart construction and OHLC", "A price chart groups observations into defined intervals. Open, high, low, and close values describe what occurred within an interval; the selected timeframe changes the detail visible to the learner.", ["OHLC", "candlestick", "bar chart", "timeframe"], cftcGlossary),
      topic("timeframe-aggregation", "Timeframes and aggregation", "A one-minute, daily, and weekly chart can describe the same instrument at different levels of aggregation. Compare the question, lookback period, and market-session context before combining observations.", ["timeframe", "aggregation", "multi-timeframe", "session"], cmeTechnical),
      topic("swing-structure", "Swing structure and trend", "Swing highs and lows provide a descriptive way to organize price structure. A sequence can change or fail, so trend language should describe observed context rather than promise continuation.", ["trend", "higher high", "higher low", "swing"], cmeTechnical),
      topic("support-resistance-zones", "Support and resistance zones", "Support and resistance are prior price areas where attention, trading activity, or directional change appeared. Treat them as zones with uncertainty, not exact floors or ceilings.", ["support", "resistance", "zone", "acceptance"], cmeTechnical),
      topic("trendlines-channels", "Trendlines and channels", "Trendlines and channels connect selected price points to help organize slope and range. Their usefulness depends on the chosen anchors and timeframe, which should be visible to the learner.", ["trendline", "channel", "slope", "range"], cmeTechnical),
      topic("ranges-breakouts", "Ranges, breakouts, and false breaks", "A range marks repeated movement between upper and lower areas. A move outside that range is an observation that may or may not gain follow-through; failed moves are part of the same uncertainty.", ["range", "breakout", "false breakout", "follow-through"], cmeTechnical),
      topic("continuation-patterns", "Continuation-pattern vocabulary", "Flags, pennants, triangles, and similar labels describe selected shapes within a prior move. The label is a way to communicate structure, not evidence that a future move is assured.", ["flag", "pennant", "triangle", "continuation"], cmeTechnical),
      topic("reversal-patterns", "Reversal-pattern vocabulary", "Head-and-shoulders, double tops, double bottoms, and related labels are descriptive pattern vocabulary. They need a defined timeframe, competing explanations, and a failure case.", ["reversal", "head and shoulders", "double top", "double bottom"], cmeTechnical),
      topic("gaps-event-context", "Gaps and event context", "A gap is a discontinuity between one interval’s close and the next interval’s open. New information, limited liquidity, and market-session structure can all affect how a learner documents the event.", ["gap", "open", "close", "event risk"], cftcGlossary),
      topic("moving-averages", "Moving averages and lag", "A moving average calculates an average over a selected past lookback period. It can summarize historical context while necessarily changing more slowly than the observations used to calculate it.", ["moving average", "lookback", "lag", "trend context"], cmeTechnical),
      topic("momentum-rate-change", "Momentum and rate of change", "Momentum tools compare recent price changes with a selected lookback. The calculation can organize the speed and direction of prior movement but does not resolve what happens next.", ["momentum", "rate of change", "lookback", "oscillator"], cmeTechnical),
      topic("rsi-context", "RSI and bounded oscillators", "The relative strength index is a bounded oscillator built from selected price changes. Read the lookback setting, market context, and difference between an indicator condition and an executable decision.", ["RSI", "oscillator", "overbought", "oversold"], cmeTechnical),
      topic("macd-context", "MACD and signal lines", "MACD compares moving-average relationships and often displays a signal line or histogram. Its output depends on the selected averages, so it should be treated as a transformed historical series rather than an instruction.", ["MACD", "signal line", "histogram", "moving average"], cmeTechnical),
      topic("stochastic-context", "Stochastic oscillator context", "A stochastic oscillator compares a selected close with a recent high-low range. It can help describe where price sits within that historical range without identifying intent or guaranteeing reversal.", ["stochastic", "high-low range", "oscillator", "lookback"], cmeTechnical),
      topic("volume-context", "Volume and participation context", "Volume measures the amount traded during a chosen period. Relative changes in volume can add context to price movement, but reported volume alone does not reveal every participant’s motive or future action.", ["volume", "relative volume", "participation", "liquidity"], cftcGlossary),
      topic("fibonacci-context", "Fibonacci retracement and extension context", "Fibonacci tools apply selected ratios to a user-defined price range. The drawn anchors and timeframe should be explicit because different inputs can create different reference levels.", ["Fibonacci", "retracement", "extension", "reference level"], cmeTechnical),
      topic("multi-timeframe-process", "Multi-timeframe review", "Multi-timeframe review compares observations across more than one aggregation level. It can surface context conflicts, but it also creates a need to document which timeframe governs the learning question.", ["multiple timeframes", "top down", "context", "time horizon"], cmeTechnical),
      topic("technical-fundamental-integration", "Technical and fundamental integration", "Technical and fundamental analysis use different information sets. A careful learner can keep price observations, reported business facts, liquidity, and risk boundaries distinct rather than forcing one method to make a complete forecast.", ["technical analysis", "fundamental analysis", "research process", "risk"], cmeTechnical),
    ],
  },
];
